...
from src.llm import build_report_prompt, draft_report, draft_similarity_report
...
def _run() -> None:
    ...
    if args.github_url and args.keyword is None:
        raise ValueError("To predict vulnerabilities from GitHub source, also enter a CVE keyword.")

    if args.keyword is None:
        raise ValueError("Enter a CVE keyword or --github-url.")

    validate_keyword(args.keyword)
    payload = NvdCollector(settings.nvd_api_key).search(args.keyword, args.limit or settings.max_results)
    records = parse_nvd_payload(payload)
    ranked_records = rank_by_keyword(records, args.keyword)
    ...
    if args.source_dir or args.github_url:
        ...
        report = draft_similarity_report(args.keyword, ranked_records, findings, len(source_files), len(patterns))
        validate_report(report)

        output_path = _write_report(settings.output_dir, f"{args.keyword}_similarity", report)
        print(f"Similarity report written: {output_path}")
        return

    report = draft_report(args.keyword, ranked_records)
    validate_report(report)

    output_path = _write_report(settings.output_dir, args.keyword, report)
    if args.show_prompt:
        print(build_report_prompt(args.keyword, ranked_records))
